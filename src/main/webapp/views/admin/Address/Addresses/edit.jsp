    ...
    <div class="col-md-6 offset-md-3 mt-4">
        <div class="card shadow-lg">
            <div class="card-header bg-dark text-white">
                <h4 class="mb-0">Chỉnh Sửa Địa Chỉ</h4>
            </div>
            <div class="card-body bg-light">
                ...
                <form action="${pageContext.request.contextPath}/admin/Address/Addresses/edit" method="post">
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select" id="userId" name="userId" required>
                            <c:forEach var="user" items="${listUser}">
                                <option value="${user.id}" ${address.user != null && address.user.id == user.id ? 'selected' : ''}>${user.fullName}</option>
                            </c:forEach>
                        </select>
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" id="addressLine" name="addressLine" value="${address.addressLine}" required>
                    </div>

                    <div class="mb-3">
                        <label for="ward" class="form-label">Phường/Xã</label>
                        <input type="text" class="form-control" id="ward" name="ward" value="${address.ward}">
                    </div>
                    ...
                    <div class="mb-3 form-check">
                        <input type="checkbox" class="form-check-input" id="isDefault" name="isDefault" value="true" ${address.isDefault ? 'checked' : ''}>
                        <label class="form-check-label" for="isDefault">Mặc Định</label>
                    </div>

                    <button type="submit" class="btn btn-primary btn-lg"
                            style="background-color:#0d47a1; border-color:#0d47a1;">Cập Nhật</button>
                    <a href="${pageContext.request.contextPath}/admin/Address/Addresses/list"
                       class="btn btn-success btn-lg"
                       style="background-color:#1b5e20; border-color:#1b5e20;">Hủy</a>
                </form>
            </div>
        </div>
    </div>
